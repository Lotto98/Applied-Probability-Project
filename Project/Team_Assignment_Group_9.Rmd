...
Sampled a person \(p\) from the town population, we define the following events:

* A = \{p\text{ read only one newspaper}\}
* B = \{p\text{ read at least two newspapers}\}
* C = \{p\text{ read at least one morning paper plus an evening pape}\}
* D = \{p\text{ do not read any newspapers}\}
* E = \{p\text{ read only one morning paper and one evening paper}\}
* I = \{p\text{ read newspaper I}\}
* II = \{p\text{ read read newspaper II}\}
* III = \{p\text{ read read newspaper III}\}
...
And finally we draw a picture in order to have a visual understanding of the subdivision of all the events:
...
\[\mathbb{P}[A_{II}] = 0.3 - 0.08 - 0.04 + 0.01 = 0.19\]
...
\[\mathbb{P}[A] = 0.01 + 0.19 + 0.00 = 0.20\]

Finally, the number of people who read only one newspaper is \(\mathbb{P}[A] \times 100000 = 0.20 \times 100000 = 20000\).

b) In order to solve the second point we look again at the drawn figure and we see that again we have all what we need. Indeed, we want to take into account all the intersections of people that read **at least** two newspapers and since each intersection consider two times the intersection of all of three newspaper we have to subtract that amount. Moreover, as the previous point, we introduce again three sub-events from the initial event \(B\), so let:
* \(B_{I \cap II \cap III^c} = \)\{\(p\)\text{ read the newspaper }I\text{ and }II\text{ but not }III\}
* \(B_{I^c \cap II \cap III} = \)\{\(p\)\text{ read the newspaper }II\text{ and }III\text{ but not }I\}
* \(B_{I \cap II^c \cap III} = \)\{\(p\)\text{ read the newspaper }III\text{ and }I\text{ but not }II\}
* \(B_{I \cap II \cap III} = \)\{\(p\)\text{ read the newspaper }I\text{, }II\text{ and }III\}
...
c) In this point we to retrieve how many people read at least one morning newspaper plus an evening newspaper, thus all of this can be re-write into the probability to read one of the morning newspapers **or** both, **and** the evening newspaper. More formally, we want to obtain:
<!--\[\mathbb{P}[C] = \mathbb{P}[(I \cup III) \cap II] = (\mathbb{P}[I] + \mathbb{P}[III] - \mathbb{P}[I \cap III]) \cap \mathbb{P}[II]\]-->
\[\mathbb{P}[C] = \mathbb{P}[(I \cup III) \cap II] = \mathbb{P}[(I \cap II) \cup (III \cap II)] \text{ by distrubutive law}\]
\[= \mathbb{P}[I \cap II] + \mathbb{P}[III \cap II] - \mathbb{P}[I \cap II \cap III]\]
\[= 0.08 + 0.04 - 0.01 = 0.11
\]
Finally, the number of people who read at least one morning newspapers plus an evening newspaper is \(\mathbb{P}[C] \times 100000 = 0.11 \times 100000 = 11000\).
...
\[\mathbb{P}[E] = \mathbb{P}\Big[\Big((I \setminus III) \cup (III \setminus I)\Big) \cap II\Big] = \mathbb{P}\Big[\Big((I \cap III^c) \cup (III \cap I^c)\Big) \cap II\Big]\]
\[= \mathbb{P}\Big[(I \cap III^c \cap II) \cup (III \cap I^c \cap II)\Big] \text{ by distrubutive law}\]
...
## \underline{Solution}
Let $D$ be the event that represents the result of the tossing, then $D = \{1,2,3,4,5,6\}$.
...
\begin{tabular}{|c|c|c|c|c|} 
 \hline
 \multicolumn{2}{|c|}{\multirow{2}{*}{\(P(x,y)\)}} & \multicolumn{2}{c|}{X} & \multirow{2}{*}{\(P_Y(y)\)}\\ \cline{3-4}
 \multicolumn{2}{|c|}{} & 0 & 1 & \\ \hline
 \multirow{2}{*}{Y} & 0 & 2/9 & 2/9 & 4/9 \\ \cline{2-5}
 & 1 & 1/9 & 4/9 & 5/9 \\ \hline
\multicolumn{2}{|c|} {\(P_X(x)\)} & 3/9 & 6/9 & 9/9 \\ \hline
\end{tabular}
$$

c) The total ways of getting two ones, two twos etc.. can be calculated using the permutations with repetition formula: $P_{12}^{2,2,..,2} = \frac{12!}{(2!)^6} = \frac{12!}{2^6}$. Then, in order to answer the question we must multiply this quantity by the probabilities, so we get:

$$
P[\text{each outcome exactly twice}] = \frac{12!}{2^6} \cdot \bigg(\frac{1}{3}\bigg)^6 \cdot \bigg(\frac{2}{3}\bigg)^6 
$$

d) We first calculate the probability of getting an even number or an odd number: $P[\text{even or odd}] = P[\text{even}] + P[\text{odd}] = \frac{2}{9} + \frac{1}{9} = \frac{1}{3}$. Then, using the permutations with repetition we calculate the total number of ways of getting 4 outcomes with either one or two, 4 outcomes with either three or four and 4 outcomes with either five or six: $P_{12}^{4,4,4} = \frac{12!}{4! 4!4!}$. Finally, we multiply the probabilities:
...
$$
P[X \geq 8] = pbinom(7, 12, \frac{2}{3}, lower.tail = FALSE)= 0.6315
$$
