...
Sampled a person \(p\) from the entire sample space we define the following event respect to \(p\):
...
a) To solve the first point we define three additional sub-events of event respect to person \(p\):
...
\[\mathbb{P}[C] = \mathbb{P}[(I \cup III) \cap II] = (\mathbb{P}[I] + \mathbb{P}[III] - \mathbb{P}[I \cap III]) \cdot \mathbb{P}[II]\]
...
* \(\#A\) is equal to all the possible combination of horses with the number-1 fixed in the first or second or third position, thus:
\[(6-1)! \times 3 = 360\]
...
$$
\mathbb{P}[B|W] = \frac{\mathbb{P}[W|B] \cdot \mathbb{P}[B]}{P[W|B]\cdot  \mathbb{P}[B] + \mathbb{P}[W|A]\cdot  \mathbb{P}[A]} = \frac{3/15 \cdot 1/2}{3/15 \cdot 1/2 + 5/12 \cdot 1/2} = \frac{12}{37}=0.324324324
$$
